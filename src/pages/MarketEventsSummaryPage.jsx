                        ...
                        <div style={s.toolbar}>
                            <div style={s.toolbarLeft}>
                                <div style={s.searchWrap}>
                                    ...
                                    <input
                                        type="text"
                                        placeholder="Rechercher par famille, thème ou contenu…"
                                        value={search}
                                    ...
                                    <table style={s.table}>
                                        <thead>
                                            <tr style={s.thead}>
                                                ...
                                                <th style={{ ...s.th, width: 130, textAlign: "left" }}>Famille</th>
                                        ...
                                        <tbody>
                                            {events.flatMap((event) => {
                                                ...
                                                return themes.map((t, i) => {
                                                    const rowKey = `${event.id}-${i}`;
                                                    const isHovered = hoveredRow === rowKey;
                                                    const palette = themeColor(t.theme ?? "");
                                                    const fPalette = familleColor(event.famille ?? "");
                                                    ...
                                                    return (
                                                        <tr
                                                            ...
                                                            style={{
                                                                ...
                                                                // subtle separator between documents
                                                                borderBottom: isLast
                                                                    ? "2px solid #E2E8F0"
                                                                    : "1px solid #F1F5F9",
                                                            }}
                                                        ...
                                                        >
                                                            ...
                                                            {/* ID — first theme row only */}
                                                            <td style={{ ...s.td, ...s.tdId }}
                                                                onClick={() => setSelectedEvent({ ...event, activeTheme: t })}>
                                                                {isFirst ? `#${event.id}` : ""}
                                                            </td>

                                                            {/* Famille — first theme row only */}
                                                            <td style={{ ...s.td, textAlign: "left" }}
                                                                onClick={() => setSelectedEvent({ ...event, activeTheme: t })}>
                                                                {isFirst && event.famille
                                                            ...
                                                            <td style={{ ...s.td, textAlign: "left" }}
                                                                ...
                                                                {t.theme
                                                                    ? <span style={{ ...s.themeBadge, backgroundColor: palette.bg, color: palette.color, borderColor: palette.border }}>
                                                                        {t.theme.substring(0, 48)}
                                                                    </span>
                                                                    : <span style={s.naText}>—</span>}
                                                            </td>

                                                            {/* Date — first theme row only */}
                                                            <td style={{ ...s.td, ...s.tdDate }}
                                                                onClick={() => setSelectedEvent({ ...event, activeTheme: t })}>
                                                                {isFirst ? fmtDate(event.genereLe) : ""}
                                                            </td>

                                                            {/* Summary preview */}
                                                            <td style={{ ...s.td, ...s.tdSummary }}
                                                                onClick={() => setSelectedEvent({ ...event, activeTheme: t })}>
                                                                {t.contenuFr?.substring(0, 110)}
                                                            </td>

                                                            {/* Delete — first theme row only */}
...
const s = {
    ...
    familleBadge: { display: "inline-block", padding: "3px 9px", borderRadius: "6px", fontSize: "11.5px", fontWeight: "600", border: "1px solid", maxWidth: "100%", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" },
    naText: { color: "#CBD5E1" },
